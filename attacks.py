...
class Render3D(object):
    def __init__(self, config_idx=1, count=1):
        ...
        self.lights = PointLights(device=self.device, ambient_color=((0.3, 0.3, 0.3),),
    ...
    def render(self, img):
        ...
        if texture_type == 'random_pixel':
            ...
            texture_image.data = torch.ones_like(texture_image, device=device) * (
                        torch.rand((1, 1, 1, 3), device=device) * 0.6 + 0.1)
        elif texture_type == 'custom':
            texture_image.data = torch.ones_like(texture_image, device=device) * torch.FloatTensor(
                [0 / 255., 0. / 255., 0. / 255.]).view((1, 1, 1, 3)).to(device)

        (pattern_h, pattern_w) = (
        self.model_settings[self.source_models[pick_idx]][4], self.model_settings[self.source_models[pick_idx]][3])
        ...
        (x, y) = self.model_settings[self.source_models[pick_idx]][1], \
                 self.model_settings[self.source_models[pick_idx]][2]
        texture_image[:, y:y + pattern_h, x:x + pattern_w, :] = resized_x_adv

        # Adjust the light parameters
        self.lights.location = torch.tensor(exp_settings['light_location'], device=device)[None] + (
                    torch.rand((3,), device=device) * exp_settings['rand_light_location'] - exp_settings[
                'rand_light_location'] / 2)
        self.lights.ambient_color = torch.tensor([exp_settings['ambient_color']] * 3, device=device)[None] + (
                    torch.rand((1,), device=self.device) * exp_settings['rand_ambient_color'])
        self.lights.diffuse_color = torch.tensor([exp_settings['diffuse_color']] * 3, device=device)[None] + (
                    torch.rand((1,), device=self.device) * exp_settings['rand_diffuse_color'])
        ...
        R, T = look_at_view_transform(dist=(self.model_settings[self.source_models[pick_idx]][5]) * rand_dist,
                                      elev=self.model_settings[self.source_models[pick_idx]][6] + rand_elev,
                                      azim=self.model_settings[self.source_models[pick_idx]][7] + rand_azim,
                                      up=((0, 1, 0),))
        ...
        if background_type == 'random_pixel':
            ...
            background_img = F.conv2d(background_img, self.gaussian_kernel, bias=None, stride=1, padding='same',
                                      groups=3)
            result_img = background_img * (1 - rendered_img_mask) + rendered_img * rendered_img_mask
        elif background_type == 'custom':
            background_img = torch.ones_like(rendered_img, device=device) * torch.FloatTensor(
                [0 / 255., 0. / 255., 0. / 255.]).view((1, 3, 1, 1)).to(device)
            result_img = background_img * (1 - rendered_img_mask) + rendered_img * rendered_img_mask
        else:
...
def calculate_admix(model, x_adv_or_nes, y, number_of_si_scales, target_label, attack_type, prob, loss_fn, config_idx,
                    renderer):
...
class FeatureMixup(nn.Module):
    def __init__(self, model: nn.Module, config_idx, input_size,idx):
        super().__init__()
        exp_settings = exp_configuration[config_idx]
        ...
        def hook_fn(module, input, output):
            if type(module) == torch.nn.Linear or output.size()[-1] <= self.input_size // divisor:
                ...
                if self.mixup_layer == 'conv_linear_no_last' and (layer_idx + 1) == self.layer_num and type(
                    ...
                    if layer_idx in self.outputs and self.record == False:
                        c = torch.rand(1).item()
                        if c <= self.prob:

                            if mixed_image_type_feature == 'A':
                                prev_feature = output.clone().detach()
                            else:
                                prev_feature = self.outputs[layer_idx].clone().detach()

                            if shuffle_image_feature == 'SelfShuffle':
                                idx1 = torch.randperm(output.shape[0])

                                prev_feature_shuffle = prev_feature[idx1].view(prev_feature.size())
                                del idx1
...
class LogitLoss(nn.Module):
    def __init__(self, labels, targeted=True, top_k=5 ):
        ...
        self.penalty_weights = torch.arange(top_k, 0, -1).float()
        self.penalty_weights = self.penalty_weights / self.penalty_weights.sum()
    ...
    def forward(self, logits):
        ...
        mask.scatter_(1, self.labels.unsqueeze(1), 0)


        non_target_logits = logits.masked_select(mask).view(batch_size, num_classes - 1)
        topk_non_target, _ = torch.topk(non_target_logits, self.top_k, dim=1)

        weighted_non_target = (topk_non_target * self.penalty_weights.to(logits.device)).sum(dim=1)


        dynamic_loss = (target_logits - weighted_non_target).sum()
...
def selective_frequency_mix(img_fre, re_x, a=0.00001):

    B, C, H_freq, W_freq = img_fre.shape
    ...
    for channel_idx in range(C):
        ...
        candidate = random.choice(range(B))
        for _ in range(block_count):

            block_size = random.choice([8])

            x1 = random.randint(0, W_freq - block_size)
            y1 = random.randint(0, H_freq - block_size)


            block_replace = re_x[candidate, channel_idx, y1:y1 + block_size, x1:x1 + block_size]
            mask = torch.ones_like(block_replace)

            block_replace = block_replace * mask

            img_fre[:, channel_idx, y1:y1 + block_size, x1:x1 + block_size] = (
                    (1 - a) * img_fre[:, channel_idx, y1:y1 + block_size, x1:x1 + block_size] +
                    a * block_replace
            )
    return img_fre


def advanced_fgsm(args, attack_type, source_model, x, y, target_label=None, num_iter=10, max_epsilon=16, mu=1.0,
                  number_of_v_samples=5, beta=1.5, number_of_si_scales=5, count=0, config_idx=578, save_interval=20):
    ...
    if exp_settings['targeted']:
        ...
        loss_fn = CELoss(y, targeted=False)
    ...
    for t in range(num_iter):
        if t >= consumed_iteration:
            ...
            img_fre = dct_2d(x_nes)
            img_fre_x = dct_2d(x)
            re_x = img_fre_x[idx]
            img_fre = selective_frequency_mix(img_fre, re_x, a=0.00001)
            x_nes = idct_2d(img_fre)

            selected_idx = torch.randint(0, B, (1,)).item()
            selected_x = x[selected_idx].unsqueeze(0)

            mean = selected_x.mean(dim=[1, 2]).squeeze(0)
            std = selected_x.std(dim=[1, 2]).squeeze(0)
            noise = torch.zeros_like(x_nes)
            for c in range(C):
                noise[:, c, :, :] = (torch.randn(B, H, W, device=x.device) * std[c] + mean[c]) * 0.01
            x_nes = x_nes + noise
            ...
            if 'I' in attack_type:
                ...
                value, _ = torch.sort(output2, dim=1, descending=True)

                loss = 0.2*loss_fn(output2) -0.8*loss_fn1(output2 / (torch.unsqueeze(value[:, 0] - value[:, 1], 1).detach()),
                                                   target_label)
