...
def main(args):
    ...
    if dset == 'ImageNet':
        ...
        img_size = 299
        transfer_models = [
            WrapperModel(load_model(x), mean, stddev, True if x in small_sized_models else False).to(device) for x in
            target_model_names]  # ,resize=False if x in 'inception_v3' else True
    ...
    succs_cursors = [wb.new_cursor('Succ_' + str((n + 1) * 20), "A2", 2 + len(target_model_names), overwrite=True) for n
                     in range(exp_settings['max_iterations'] // 20)]
    accs_cursors = [wb.new_cursor('Accs_' + str((n + 1) * 20), "A2", 2 + len(target_model_names), overwrite=True) for n
                    in range(exp_settings['max_iterations'] // 20)]
    ...
    for model_i, source_model_name in enumerate(source_model_names):
        ...
        def iter_source():
            num_images = 0
            target_accs = {m: {k: ([0.] * (exp_settings['max_iterations'] // 20)) for k in attack_methods.keys()} for m
                           in target_model_names}
            target_succs = {m: {k: ([0.] * (exp_settings['max_iterations'] // 20)) for k in attack_methods.keys()} for m
                            in target_model_names}
            ...
            for k in range(0, num_batches):
                ...
                for i in range(batch_size_cur):
                    if dset == 'ImageNet':
                        ...
                        img[i] = trn(
                            Image.open("./dataset/CIFAR10_targeted/" + image_id_list[k * batch_size + i] + '.png'))
                ...
                output_dict = {key: advanced_fgsm(args, atk, source_model, img, labels, target_labels,
                                                  num_iter=exp_settings['max_iterations'],
                                                  max_epsilon=exp_settings['epsilon'], count=k,
                                                  config_idx=args.config_idx) for key, atk in
                ...
                for key, value in output_dict.items():
                    ...
                    for nn in range(imgs.size()[0]):
                        plot_img(imgs[nn],
                                 str(args.config_idx) + '/' + source_model_name + '/' + str(k * batch_size + nn))
                    ...
                    for n in range(exp_settings['max_iterations'] // 20):
                        ...
                        for a in attack_methods.keys():
                            ...
                            if n == exp_settings['max_iterations'] // 20 - 1:
                                succ = (target_succs[target_model_names[j]][a][
                                    exp_settings['max_iterations'] // 20 - 1]) / num_images
                                acc = (target_accs[target_model_names[j]][a][
                                    exp_settings['max_iterations'] // 20 - 1]) / num_images
                                ...
                                avg_acc += acc * 100
                                print(
                                    f'[{k * batch_size + batch_size_cur}/{len(image_id_list)}]Success Rate (%) on {target_model_names[j]} with {a} : {succ * 100:.2f} | Acc (%) : {acc * 100:.2f}',
                                    flush=True)
        ...
        for a in attack_methods.keys():  # Export experimental results
            for n in range(exp_settings['max_iterations'] // 20):
                ...
                for j, mod in enumerate(transfer_models):
                    ...
                    if args.neptune:
                        run['Final_Succ_' + str((n + 1) * 20) + '/' + source_model_name + "/" + target_model_names[
                            j]] = final_succ * 100
                        run['Final_Acc_' + str((n + 1) * 20) + '/' + source_model_name + "/" + target_model_names[
                            j]] = final_acc * 100
...
def argument_parsing():
    ...
    parser.add_argument("--config_idx", default=578, type=int, help="experiment config index")
    parser.add_argument("--neptune", default=False, type=bool, help="experiment config index")
    parser.add_argument("--clean", default=1, type=int, help="experiment config index")
    parser.add_argument("--m", default="fre3", type=str, help=" ")
    return parser
